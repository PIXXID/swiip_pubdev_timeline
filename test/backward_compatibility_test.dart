...
void main() {
  group('Timeline Backward Compatibility Property Tests', () {
    ...
    test(
        'Property 8: Backward Compatibility - Timeline without config uses default values',
        () {
      ...
      for (var iteration = 0; iteration < 100; iteration++) {
        ...
        expect(config.dayWidth, equals(45.0),
            reason: 'Default dayWidth should be 45.0');
        expect(config.dayMargin, equals(5.0),
            reason: 'Default dayMargin should be 5.0');
        expect(config.datesHeight, equals(65.0),
            reason: 'Default datesHeight should be 65.0');
        expect(config.timelineHeight, equals(300.0),
            reason: 'Default timelineHeight should be 300.0');
        expect(config.rowHeight, equals(30.0),
            reason: 'Default rowHeight should be 30.0');
        expect(config.rowMargin, equals(3.0),
            reason: 'Default rowMargin should be 3.0');
        expect(config.bufferDays, equals(5),
            reason: 'Default bufferDays should be 5');
    ...
    test(
        'Property 8: Backward Compatibility - Programmatic configuration overrides defaults',
        () {
      // Feature: external-configuration-system, Property 8: Backward Compatibility
      // Validates: Requirements 8.1, 8.3
      ...
      for (var iteration = 0; iteration < 100; iteration++) {
        ...
        final customConfig = TimelineConfiguration(
          ...
          scrollThrottleDuration:
              Duration(milliseconds: 10 + random.nextInt(80)), // 10-90ms
          animationDuration:
              Duration(milliseconds: 150 + random.nextInt(300)), // 150-450ms
        );
        ...
        expect(config.dayWidth, equals(customConfig.dayWidth),
            reason: 'dayWidth should match programmatic config');
        expect(config.dayMargin, equals(customConfig.dayMargin),
        ...
        expect(config.rowHeight, equals(customConfig.rowHeight),
            reason: 'rowHeight should match programmatic config');
        expect(config.rowMargin, equals(customConfig.rowMargin),
        ...
        expect(config.scrollThrottleDuration,
            equals(customConfig.scrollThrottleDuration),
