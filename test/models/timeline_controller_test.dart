...
void main() {
  group('TimelineController Property Tests', () {
    test(
        'Property 6: Scroll Throttling - listener callbacks invoked at most 60 times per second',
        () async {
      ...
      for (var iteration = 0; iteration < 100; iteration++) {
        ...
        final viewportWidth =
            600.0 + random.nextDouble() * 600; // 600-1200 viewport
        ...
        for (var i = 0; i < numberOfScrollUpdates; i++) {
          final offset =
              random.nextDouble() * (totalDays * (dayWidth - dayMargin));
        ...
        final expectedMaxUpdates =
            (totalTime.inMilliseconds / 16).ceil() + 10; // +10 for tolerance
        ...
        expect(
          ...
          reason:
              'Updates should be throttled (got $updateCount, sent $numberOfScrollUpdates)',
        );
        ...
        expect(
          ...
          reason:
              'Updates should not exceed ~60 per second (got $updateCount in ${totalTime.inMilliseconds}ms, expected max $expectedMaxUpdates)',
        );
    ...
    test(
        'Property 6: Scroll Throttling - rapid updates do not cause excessive calculations',
        () async {
      ...
      for (var iteration = 0; iteration < 100; iteration++) {
        ...
        expect(
          ...
          lessThan(50),
          reason:
              'Center index updates should be throttled (got $centerIndexUpdateCount from 50 scroll updates)',
        );
  ...
  group('TimelineController Resource Cleanup Property Tests', () {
    ...
    test(
        'Property 4: Resource Cleanup - throttle timer is cancelled on disposal',
        () async {
      ...
      for (var iteration = 0; iteration < 100; iteration++) {
        ...
        expect(
          ...
          reason:
              'No updates should occur after disposal (timer should be cancelled)',
        );
      }
    }, timeout: const Timeout(Duration(seconds: 60)));
  ...
  group('TimelineController Unit Tests', () {
    ...
    test('visible range includes buffer', () async {
      ...
      final expectedStart =
          (centerIndex - (visibleDays ~/ 2) - buffer).clamp(0, 100);
      final expectedEnd =
          (centerIndex + (visibleDays ~/ 2) + buffer).clamp(0, 100);
