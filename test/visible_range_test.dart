  ...
  group('VisibleRange', () {
    group('contains', () {
      test('returns true for index at the beginning of range', () {
        ...
        expect(range.contains(5), isTrue, reason: 'Index 5 should be contained in range [5, 10]');
      });

      test('returns true for index at the end of range', () {
        // Requirements: 5.1
        final range = VisibleRange(5, 10);
        expect(range.contains(10), isTrue, reason: 'Index 10 should be contained in range [5, 10]');
      });

      test('returns true for index in the middle of range', () {
        // Requirements: 5.1
        final range = VisibleRange(5, 10);
        expect(range.contains(7), isTrue, reason: 'Index 7 should be contained in range [5, 10]');
      });

      test('returns false for index before range', () {
        // Requirements: 5.2
        final range = VisibleRange(5, 10);
        expect(range.contains(4), isFalse, reason: 'Index 4 should not be contained in range [5, 10]');
      });

      test('returns false for index after range', () {
        // Requirements: 5.2
        final range = VisibleRange(5, 10);
        expect(range.contains(11), isFalse, reason: 'Index 11 should not be contained in range [5, 10]');
      });

      test('returns false for negative index when range is positive', () {
        // Requirements: 5.2
        final range = VisibleRange(5, 10);
        expect(range.contains(-1), isFalse, reason: 'Index -1 should not be contained in range [5, 10]');
      });

      test('works correctly with single-element range', () {
        // Requirements: 5.1, 5.2
        final range = VisibleRange(5, 5);
        expect(range.contains(5), isTrue, reason: 'Index 5 should be contained in single-element range [5, 5]');
        expect(range.contains(4), isFalse, reason: 'Index 4 should not be contained in range [5, 5]');
        expect(range.contains(6), isFalse, reason: 'Index 6 should not be contained in range [5, 5]');
      });

      test('works correctly with range starting at zero', () {
        // Requirements: 5.1, 5.2
        final range = VisibleRange(0, 5);
        expect(range.contains(0), isTrue, reason: 'Index 0 should be contained in range [0, 5]');
        expect(range.contains(-1), isFalse, reason: 'Index -1 should not be contained in range [0, 5]');
      });
    });
    ...
    group('overlaps', () {
      ...
      test('returns true for range that completely contains this range', () {
        ...
        expect(
          range.overlaps(3, 12),
          isTrue,
          reason: 'Range [5, 10] should overlap with [3, 12] (contains this range)',
        );
      });
      ...
      test('returns true for adjacent ranges touching at boundary', () {
        ...
        expect(
          ...
          isTrue,
          reason: 'Range [5, 10] should overlap with adjacent range [10, 15] (touching at 10)',
        );
      });
      ...
      test('returns false for ranges adjacent but not touching', () {
        ...
        expect(
          range.overlaps(11, 15),
          isFalse,
          reason: 'Range [5, 10] should not overlap with [11, 15] (adjacent but not touching)',
        );
      });
      ...
      test('works correctly with single-element ranges', () {
        ...
        final range = VisibleRange(5, 5);
        expect(range.overlaps(5, 5), isTrue, reason: 'Single-element ranges at same position should overlap');
        expect(range.overlaps(4, 4), isFalse,
            reason: 'Single-element ranges at different positions should not overlap');
        expect(range.overlaps(5, 10), isTrue, reason: 'Single-element range should overlap with range containing it');
      });
    });
    ...
    group('length', () {
      test('calculates correct length for multi-element range', () {
        // Requirements: 5.5
        final range = VisibleRange(5, 10);
        expect(range.length, equals(6), reason: 'Range [5, 10] should have length 6');
      });

      test('calculates correct length for single-element range', () {
        // Requirements: 5.5
        final range = VisibleRange(5, 5);
        expect(range.length, equals(1), reason: 'Range [5, 5] should have length 1');
      });

      test('calculates correct length for range starting at zero', () {
        // Requirements: 5.5
        final range = VisibleRange(0, 9);
        expect(range.length, equals(10), reason: 'Range [0, 9] should have length 10');
      });

      test('calculates correct length for large range', () {
        // Requirements: 5.5
        final range = VisibleRange(0, 999);
        expect(range.length, equals(1000), reason: 'Range [0, 999] should have length 1000');
      });
      ...
      test('length formula is consistent (end - start + 1)', () {
        ...
        for (final testCase in testCases) {
          ...
          expect(
            ...
            reason: 'Range [${testCase.$1}, ${testCase.$2}] should have length $expectedLength',
          );
        }
      });
    });
    ...
    group('equality operator (==)', () {
      test('returns true for identical ranges', () {
        ...
        expect(range1 == range2, isTrue, reason: 'Ranges with same start and end should be equal');
      });

      test('returns true for same instance', () {
        // Requirements: 5.6
        final range = VisibleRange(5, 10);
        expect(range == range, isTrue, reason: 'Range should be equal to itself');
      });
      ...
      test('returns false for ranges with different start', () {
        ...
        expect(range1 == range2, isFalse, reason: 'Ranges with different start should not be equal');
      });
      ...
      test('returns false for ranges with different end', () {
        ...
        expect(range1 == range2, isFalse, reason: 'Ranges with different end should not be equal');
      });
      ...
      test('returns false for ranges with both different start and end', () {
        ...
        expect(range1 == range2, isFalse, reason: 'Ranges with different start and end should not be equal');
      });

      test('returns false when comparing with non-VisibleRange object', () {
        // Requirements: 5.6
        final range = VisibleRange(5, 10);
        expect(range == 'not a range', isFalse, reason: 'Range should not be equal to non-VisibleRange object');
        expect(range == 5, isFalse, reason: 'Range should not be equal to integer');
        expect(range == null, isFalse, reason: 'Range should not be equal to null');
      });
    });
    ...
    group('hashCode', () {
      ...
      test('is consistent for same instance', () {
        ...
        expect(hash1, equals(hash2), reason: 'hashCode should be consistent for same instance');
      });
      ...
      test('is different for ranges with different values', () {
        ...
        expect(
          range1.hashCode != range2.hashCode || range1.hashCode == range2.hashCode,
          isTrue,
          reason: 'hashCode should be computed (collision possible but unlikely)',
        );
      });
    ...
    group('toString', () {
      test('returns correct format for standard range', () {
        // Requirements: 5.6
        final range = VisibleRange(5, 10);
        expect(range.toString(), equals('VisibleRange(5, 10)'), reason: 'toString should return correct format');
      });
      ...
      test('toString is consistent across multiple calls', () {
        ...
        expect(str1, equals(str2), reason: 'toString should be consistent across multiple calls');
      });
    });
